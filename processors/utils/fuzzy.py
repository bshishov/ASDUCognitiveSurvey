...
class FuzzySetCompound(FuzzySet):
    def __init__(self, *args):
        super(FuzzySetCompound, self).__init__()
        self.args = args
...
class ConstMf(MembershipFunction):
    def __init__(self, value):
        super(ConstMf, self).__init__(name='const')
        self.value = value
...
class TriangularMf(MembershipFunction):
    def __init__(self, name, left, support, right):
        super(TriangularMf, self).__init__(name=name)
        self.left = left
...
class TrapezoidalMf(MembershipFunction):
    def __init__(self, name, left, left_support, right_support, right):
        super(TrapezoidalMf, self).__init__(name=name)
        self.left = left
...
class GaussianMf(MembershipFunction):
    def __init__(self, name, mean, std):
        super(GaussianMf, self).__init__(name=name)
        self.mean = mean
        self.std = std
...
class Variable(object):
    def __init__(self, name=None, bounds=(0, 1), *terms):
        self.membership_functions = {}
        for term in terms:
            if isinstance(term, MembershipFunction):
                if term.name in self.membership_functions:
                    ValueError('Membership function with name {} is already added to this variable'.format(term.name))
